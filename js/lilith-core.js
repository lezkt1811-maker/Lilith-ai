...
(function () {
  ...
  const els = {
    ...
    rateSlider: $('#rate-slider'), rateValue: $('#rate-value'),
    pitchSlider: $('#pitch-slider'), pitchValue: $('#pitch-value'),
  ...
  function speakIfEnabled(text) {
    ...
    const voices = LilithVoice.getVoices();
    ...
    LilithVoice.speak(text, {
      voice, rate: settings.rate, pitch: settings.pitch, volume: settings.volume, muted
    }, {
  ...
  if (LilithVoice.synthAvailable) {
    populateVoiceOptions();
    window.speechSynthesis.onvoiceschanged = populateVoiceOptions;
  } else {
    els.voiceSelect.innerHTML = '<option>Speech synthesis unavailable</option>';
    els.voiceSelect.disabled = true;
  }

  els.voiceSelect.addEventListener('change', () => { settings.voiceURI = els.voiceSelect.value; saveSettings(); });

  bindSlider(els.rateSlider, els.rateValue, 'rate', v => v.toFixed(2));
  bindSlider(els.pitchSlider, els.pitchValue, 'pitch', v => v.toFixed(2));
  bindSlider(els.volumeSlider, els.volumeValue, 'volume', v => Math.round(v * 100) + '%');
